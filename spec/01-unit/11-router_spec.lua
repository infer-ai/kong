...
describe("Router", function()
  ...
  describe("exec()", function()
    ...
    it("returns matched_host + matched_uri + matched_method", function()
      ...
      local match_t = router.exec(_ngx)
      assert.same(use_case_apis[1], match_t.api)
      assert.equal("host.com", match_t.matched_host)
      assert.equal("/my-api", match_t.matched_uri)
      assert.equal("GET", match_t.matched_method)
      ...
      assert.equal("host.com", match_t.matched_host)
      assert.equal("/my-api", match_t.matched_uri)
      assert.is_nil(match_t.matched_method)

      _ngx = mock_ngx("GET", "/", { ["host"] = "test.host.com" })
      match_t = router.exec(_ngx)
      assert.same(use_case_apis[3], match_t.api)
      assert.equal("*.host.com", match_t.matched_host)
      assert.is_nil(match_t.matched_uri)
      assert.is_nil(match_t.matched_method)

      _ngx = mock_ngx("GET", "/users/123/profile", {})
      match_t = router.exec(_ngx)
      assert.same(use_case_apis[4], match_t.api)
      assert.is_nil(match_t.matched_host)
      assert.equal([[/users/\d+/profile]], match_t.matched_uri)
      assert.is_nil(match_t.matched_method)
    end)
    ...
    it("returns uri_captures from a [uri regex]", function()
      ...
      local match_t = router.exec(_ngx)
      assert.equal("1984", match_t.uri_captures[1])
      assert.equal("1984", match_t.uri_captures.user_id)
      assert.equal("",     match_t.uri_captures[2])
      assert.equal("",     match_t.uri_captures.scope)
      -- returns the full match as well
      assert.equal("/users/1984/profile", match_t.uri_captures[0])
      -- no stripped_uri capture
      assert.is_nil(match_t.uri_captures.stripped_uri)
      assert.equal(2, #match_t.uri_captures)
      ...
      assert.equal("1984",  match_t.uri_captures[1])
      assert.equal("1984",  match_t.uri_captures.user_id)
      assert.equal("email", match_t.uri_captures[2])
      assert.equal("email", match_t.uri_captures.scope)
      -- returns the full match as well
      assert.equal("/users/1984/profile/email", match_t.uri_captures[0])
      -- no stripped_uri capture
      assert.is_nil(match_t.uri_captures.stripped_uri)
      assert.equal(2, #match_t.uri_captures)
    end)
    ...
    it("returns no uri_captures from a [uri prefix] match", function()
      ...
      assert.is_nil(match_t.uri_captures)
    end)
